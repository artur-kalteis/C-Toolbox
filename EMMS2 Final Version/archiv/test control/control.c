...
void json_parse_influxdb(json_object *jobj)
	...
	int x,p;
        ...
        enum influx_structur i_struct=0;        
                          ...
                          if(i_struct==3)
                          {
                           ...
                           // printf("value  %d\n", json_object_get_int(jvalue));
                            ...
                            // printf("testing  --------------%i-\n",buffer_count);
                              ...
                              //  printf(" %s  und %s  comparing\n",aib_name_buffer[buffer_count],m_->slave_objekt[act_slave_count].output_cmd->output_cmd_array[ix].name);
                                if(!strcmp(aib_name_buffer[buffer_count],m_->slave_objekt[act_slave_count].output_cmd->output_cmd_array[ix].name))
                                {
...
/********************************shared Memory Parameter*****************************************/
  ...
  while(1)
  {
    ...
    {   
      ...
      sprintf(query_read, "q=SELECT * from %s",m_->slave_objekt[i1].slave_par_id);
    ...
    { 
      if(!strcmp(slave_name[i1],"AB1001"))
      {
        ...
        sprintf(query_read, "q=SELECT * from %s",m_->slave_objekt[i1].slave_cmd_id);
